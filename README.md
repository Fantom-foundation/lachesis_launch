...
The latest version of a blockchain node for the Opera network is `go-opera 1.1.2-rc.4`. The oldest supported version is `go-opera 1.1.1-rc.2`.
...
Read node can use `go-opera v1.1.2-rc.4` in either snap sync or full sync mode.

1. [Launching go-opera readonly node](docs/setup-readonly-node.sh)

### Instructions for an API node

API node can use `go-opera v1.1.2-rc.4` in either snap sync or full sync mode.
...
Validator node is recommended to use `go-opera v1.1.2-rc.4` in full sync mode.
